...
int main(void){
  ...
  while(1) {
    ...
    Led = GPIO_PORTF_DATA_R;   // read previous
		// If both PF4 and PF0 switch are not pressed, the PF1 output should be low.
		if (!SW1 || !SW2) {
			GPIO_PORTF_DATA_R ^= 0x02;     // Toggle LED on port 2
			if(i<50){
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
				Data[i] = (0|(GPIO_PORTF_DATA_R|0x10) | (GPIO_PORTF_DATA_R|0x02) | (GPIO_PORTF_DATA_R|0x01)); // record PF1
				last = now;
				i++;
			}
		} else {
			GPIO_PORTF_DATA_R &= ~0x02;           // Disable LED on port 2
		}
		Delay();
  }
}
